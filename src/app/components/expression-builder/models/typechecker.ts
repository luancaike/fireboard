...
export function typeCheck(cst, rootType) {
    class TypeChecker extends ExpressionVisitor {
        ...
        caseExpression(ctx) {
            ...
            if (args.length < 2) {
                this.errors.push({ message: `CASE expects 2 arguments or more` });
                return [];
            }
        ...
        functionExpression(ctx) {
            ...
            if (!clause.multiple && clause.args.length !== args.length) {
                const message = `Function ${name} expects ${expectedArgsLength} argument`;
                this.errors.push({ message });
            } else {
                return args.map((arg, index) => {
                    const argType = clause.args[index];
                    const genericType = argType === 'number' || argType === 'string' ? 'expression' : argType;
        ...
        numberLiteral(ctx) {
            ...
            if (type === 'boolean') {
                ...
                const message = `Expecting boolean but found ${literal}`;
                this.errors.push({ message });
            }
        }
    ...
    switch (name) {
        ...
        case 'string':
            if (children.expression) {
                ...
                return expression.length === 1 ? expression[0] : { name, children: { expression: expression } };
            }
            break;
        ...
        case 'relationalExpression':
            if (children.operands) {
                const operands = children.operands.map(compactSyntaxTree);
                const operators = children.operators;
                return operands.length === 1 ? operands[0] : { name, children: { operators, operands } };
            }
            break;
        ...
        case 'caseExpression': {
            ...
            const args = children.arguments ? children.arguments.map(compactSyntaxTree) : [];
            return {
                name,
