...
function tokenNode(...args: any) {
    // eslint-disable-next-line prefer-const
    let [type, token] = args.length === 1 ? ['token', args[0]] : args;
    // allow passing the array token
    if (Array.isArray(token)) {
        if (token.length !== 1) {
            console.warn(`Passed token array of length ${token.length} to tokenNode()`, token);
        }
        token = token[0];
    }
    return (
...
export class ExpressionSyntaxVisitor extends ExpressionCstVisitor {
    ...
    logicalNotExpression(ctx) {
        return syntaxNode('filter', tokenNode(ctx.operators), this.visit(ctx.operands));
    }
    ...
    _logicalExpression(operands = [], operators = []) {
        ...
        return initial.length === 0 ? null : initial.length === 1 ? initial[0] : syntaxNode('filter', ...initial);
    }
    ...
    _arithmeticExpression(operands = [], operators = []) {
        ...
        return initial.length === 0 ? null : initial.length === 1 ? initial[0] : syntaxNode('math', ...initial);
    }
    ...
    functionExpression(ctx) {
        ...
        if (ctx.LParen) {
            ...
            parts.push(
                syntaxNode('group', tokenNode('open-paren', ctx.LParen), ...args, tokenNode('close-paren', ctx.RParen))
            );
        }
    ...
    numberLiteral(ctx) {
        return syntaxNode('number-literal', tokenNode(ctx.Minus), tokenNode(ctx.NumberLiteral));
    }
    atomicExpression(ctx) {
        return this.visit(ctx.expression);
    }
    parenthesisExpression(ctx) {
        return syntaxNode('group', tokenNode(ctx.LParen), this.visit(ctx.expression), tokenNode(ctx.RParen));
    }
}
...
export function defaultParser(options) {
    ...
    return visited && recoverTokens(visited, options.source, options.recover ? recoveredNode : recoveredWhitespaceNode);
}
...
export function fallbackParser({ source, startRule }) {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        } else if (isTokenType(t.tokenType, Identifier) || isTokenType(t.tokenType, IdentifierString)) {
...
function recoverTokens(root, source, recovered = recoveredNode) {
    const getRecoveredToken = (start, end) => recovered(source.substring(start, end), { start, end: end - 1 });
...
export function syntax({ strategies = DEFAULT_STRATEGIES, ...options }: any = {}) {
