...
export const EXPRESSION_TYPES = ['expression', 'aggregation', 'boolean', 'string', 'number'];
...
export const isExpressionType = (typeA, typeB) =>
    typeA === typeB || (EXPRESSION_SUBTYPES[typeB] && EXPRESSION_SUBTYPES[typeB].has(typeA)) || false;
...
for (const [name, clause] of Object.entries(MBQL_CLAUSES) as any) {
...
const MBQL_TO_EXPRESSION_NAME = new Map(
    Object.entries(MBQL_CLAUSES).map(([mbql, { displayName }]) => [mbql, displayName])
);
...
export const FUNCTIONS = new Set([...EXPRESSION_FUNCTIONS, ...AGGREGATION_FUNCTIONS]);
