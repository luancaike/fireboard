...
function createClauseToken(name, options = {}) {
    ...
    if (!tokenName) {
        throw new Error(`MBQL_CLAUSE: ${clause} is missing a tokenName or displayName`);
    }
    const token = createToken({
        name: tokenName,
        pattern: new RegExp(escape(displayName), 'i'),
        longer_alt: (Identifier.PATTERN as any).test(displayName) ? Identifier : null,
        ...options
    });
...
const quotedStringRegex = (character, closed = true) => {
    ...
    return new RegExp(`${open}(?:[^\\\\${character}]|\\\\(?:[bfnrtv${character}\\\\/]|u[0-9a-fA-F]{4}))*${close}`);
};
...
export const isTokenType = memoize(
    (child, ancestor) => {
        return child === ancestor || child.CATEGORIES.some((token) => isTokenType(token, ancestor));
    },
