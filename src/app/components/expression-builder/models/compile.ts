    ...
    caseExpression(ctx) {
        ...
        for (let i = 0; i < args.length; i++) {
            if (i === args.length - 1) {
                ...
                mbql.push({ default: args[i] } as any);
            } else {
...
export function compile({ cst, ...options }) {
    ...
    function resolveMBQLField(kind, name) {
        if (kind === 'metric') {
            const metric = parseMetric(name, options as any);
            if (!metric) {
                throw new Error(`Unknown Metric: ${name}`);
            }
            return ['metric', metric.id];
        } else if (kind === 'segment') {
            const segment = parseSegment(name, options as any);
            if (!segment) {
            ...
            // fallback
            const dimension = parseDimension(name, options as any);
            if (!dimension) {
        ...
        if (top) {
            if (top[0] === operator) {
                ...
                if (b < a) {
                    ...
                    do {
                        stack.pop();
                    } while (stack.length > 0 && OPERATOR_PRECEDENCE[stack[stack.length - 1][0]] > a);
                    if (stack.length > 0) {
