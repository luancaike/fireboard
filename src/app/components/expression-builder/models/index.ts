...
import { EDITOR_FK_SYMBOLS, EDITOR_QUOTES, FUNCTIONS, getMBQLName, OPERATORS } from './config';
...
export function formatIdentifier(name, { quotes = EDITOR_QUOTES } = {}) {
    if (!quotes.identifierAlwaysQuoted && /^\w+$/.test(name) && !isReservedWord(name)) {
        return name;
    }
...
export function parseMetric(metricName, { query }) {
    return query.table().metrics.find((metric) => metric.name.toLowerCase() === metricName.toLowerCase());
}
...
export function parseSegment(segmentName, { query }) {
    return query.table().segments.find((segment) => segment.name.toLowerCase() === segmentName.toLowerCase());
}
 ...
 */
export function getDimensionName(dimension, separator = EDITOR_FK_SYMBOLS.default) {
    return dimension.render().replace(` ${FK_SYMBOL} `, separator);
}

// STRING LITERALS

export function formatStringLiteral(mbqlString, { quotes = EDITOR_QUOTES } = {}) {
...
export function isMetric(expr) {
    return Array.isArray(expr) && expr[0] === 'metric' && expr.length === 2 && typeof expr[1] === 'number';
}

export function isSegment(expr) {
    return Array.isArray(expr) && expr[0] === 'segment' && expr.length === 2 && typeof expr[1] === 'number';
}
