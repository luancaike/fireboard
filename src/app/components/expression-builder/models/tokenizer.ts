...
export function tokenize(expression) {
    ...
    const scanStringLiteral = () => {
        ...
        const error = terminated ? null : `Missing closing quotes`;
    ...
    const scanBracketIdentifier = () => {
        ...
        while (index < length) {
            ...
            if (ch === ']') {
                ...
                const end = index;
                const error = `Bracket identifier in another bracket identifier`;
                return { type, start, end, error };
            } else if (ch === '\\') {
        ...
        const error = terminated ? null : `Missing a closing bracket`;
        return { type, start, end, error };
    };
        ...
        while (index < length) {
            ...
            if (token) {
                ...
                if (char === ']') {
                    ...
                    const message = `Invalid character: ${char}`;
                    errors.push({ message, pos });
                }
                ++index;
            }
...
export function countMatchingParentheses(tokens) {
    ...
    const count = (c, token) => (isOpen(token) ? c + 1 : isClose(token) ? c - 1 : c);
